...
func HandleRobots(w http.ResponseWriter, r *http.Request) {
	robotsFile, _ := meta.FS().ReadFile("robots.txt")
	w.Header().Set("Content-Type", "text/plain")
	_, _ = w.Write(robotsFile)
}

func HandleSitemap(w http.ResponseWriter, r *http.Request) {
	robotsFile, _ := meta.FS().ReadFile("sitemap.xml")
	w.Header().Set("Content-Type", "text/plain")
	_, _ = w.Write(robotsFile)
}

func HandleFavicon(w http.ResponseWriter, r *http.Request) {
	faviconFile, _ := assets.FS().ReadFile("favicon.ico")
...
func AssetsHandler(minifyer *minify.M) http.Handler {
	...
	case "prod":
		handler.Handle("/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			minifyer.Middleware(http.FileServer(http.FS(assets.FS()))).
				ServeHTTP(w, r)
		}))
	case "beta":
		handler.Handle("/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			http.FileServer(http.FS(assets.FS())).
				ServeHTTP(w, r)
		}))
	default:
		handler.Handle("/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
